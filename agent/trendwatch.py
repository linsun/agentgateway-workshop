...
SYSTEM_PROMPT = (
    ...
    "The tools return whatever is trending, which is often not about AI at all: general "
    "tech, politics, health, business, and so on. The tool's topic tags cannot be relied on "
    "for this -- in live mode every item is tagged only with a placeholder. So judge each "
    "item yourself, from its title and content, and before writing the digest keep ONLY the "
    "items about agentic AI or AI more broadly: LLMs, AI agents, model training or "
    "inference, AI tooling and frameworks, and infrastructure whose main story is serving "
    "or running AI models. Silently drop every item that is not about AI -- do not list it, "
    "and do not mention that you dropped it.\n"
    "If none of the trending items are about AI, do not pad the list with unrelated ones: "
    "say in a single line that nothing trending right now is about agentic AI, and stop.\n"
